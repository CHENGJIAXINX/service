...
import org.tio.sitexxx.service.model.main.UserCoinItem;
...
public class RechargeAtom extends AbsAtom {
	...
	public boolean run() throws SQLException {
		...
		if (Objects.equals(rechargeItem.getPaytype(), RechargeItem.Paytype.APPLE_APP)) {
			addCoin = new Double(NumberUtil.mul(amount.doubleValue(), ConfService.getFloat(Const.ConfMapping.CNY_TO_COIN_TIME_IOS, 70f).floatValue()));
		} else {
			addCoin = new Double(NumberUtil.mul(amount.doubleValue(), ConfService.getFloat(Const.ConfMapping.CNY_TO_COIN_TIME_PC, 100f).floatValue()));
		}
		...
		if (Objects.equals(rechargeItem.getPaytype(), RechargeItem.Paytype.APPLE_APP)) {
			userCoinItem.setAddexp(new Double(NumberUtil.mul(amount.doubleValue(), ConfService.getFloat(Const.ConfMapping.CNY_TO_COIN_EXP_TIME_IOS, 50f).floatValue())));
		} else {
			userCoinItem.setAddexp(new Double(NumberUtil.mul(amount.doubleValue(), ConfService.getFloat(Const.ConfMapping.CNY_TO_COIN_EXP_TIME_ST, 50f).floatValue())));
		}
