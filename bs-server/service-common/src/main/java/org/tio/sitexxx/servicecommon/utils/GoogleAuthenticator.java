...
public class GoogleAuthenticator {
    ...
    public static final String SEED = "g8GjEvTbW5oVSV7avLBdwIHqGlUYNzKFI17izOF8GwLDVKs2m0QN7vxRs2im5MDaNCWGmcD2rvcZx";
    ...
    /**
     * 流程：
     * 1、调用generateSecretKey生成一个secret，业务上与用户绑定
     * 2、用户名与secret通过安全途径发给用户（微信、线下等）
     * 3、用户把secret与用户名，输入到谷歌验证器，即可即时获取验证码
     * 4、用户提交验证码
     * 5、从数据库查出用户的key，与验证码通过check_code进行验证
     * 6、得出验证结果
     */

    public static void main(String[] args) {
        String secret = GoogleAuthenticator.generateSecretKey();
        String url = GoogleAuthenticator.getQRBarcodeURL("aaa", "admin.geda.com", secret);
        ...
        boolean r = ga.check_code("user的secret", code);
